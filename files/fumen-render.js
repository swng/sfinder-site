const { decoder, encoder, Field, Page } = require('tetris-fumen');
...
function fumenrender(fumenCodes, container, comments = undefined) {
    ...
    fumenCodes.forEach((code, index) => {
        try {
            ...
            if (pages.length == 1) {
                let canvas = fumen_draw(pages[0], cellSize, undefined, transparency_fumen);
            ...
            if (pages.length > 1) {
                let gif = fumen_drawFumens(pages, cellSize, undefined, start, end, transparency_fumen);
